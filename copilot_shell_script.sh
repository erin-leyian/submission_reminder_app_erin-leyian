...
CONFIG_FILE=/root/summative/submission_reminder_erin/config/config.env
...
if [[ "$answer" =~ ^[Yy]$ ]]; then
	if [[ -f "/root/summative/submission_reminder_erin/startup.sh" ]]; then
		bash /root/summative/submission_reminder_erin/startup.sh
	else
